...
if ( ! function_exists( 'besa_dokan_description' ) ) {
    function besa_dokan_description( $description ) {
        if( !besa_woo_is_vendor_page() ) return $description;
    ...
    add_filter( 'the_content', 'besa_dokan_description', 10, 1 );
}

if ( ! function_exists( 'tbay_get_sidebar_dokan' ) ) {
    function tbay_get_sidebar_dokan() {
        $sidebar = array();
        $sidebar['id'] =  besa_tbay_get_config('product_archive_sidebar');
        return $sidebar;
    }
}