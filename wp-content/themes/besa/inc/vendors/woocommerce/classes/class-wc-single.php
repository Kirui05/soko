...
if ( ! class_exists( 'Besa_Single_WooCommerce' ) ) :
	...
	class Besa_Single_WooCommerce  {
		...
		public function __construct() {
			...
			/*After meta single product*/

			add_action('woocommerce_single_product_summary', array( $this, 'besa_after_meta_single_product'), 99);
			
 
    		if(!wp_is_mobile() ) {
			    add_action( 'woocommerce_before_single_product', array( $this, 'the_sticky_menu_bar'), 30 );

			    add_action( 'besa_sticky_menu_bar_product_summary', 'besa_woocommerce_single_title', 5 );
			...
			add_action('woocommerce_before_add_to_cart_button', array( $this, 'before_add_to_cart_form'), 98);
			add_action('woocommerce_after_add_to_cart_button', array( $this, 'close_after_add_to_cart_form'), 98);
			...
			add_filter( 'woocommerce_single_product_image_thumbnail_html', array($this,'get_video_audio_content_first'), 10, 2 );
  			...
  			add_action('besa_woocommerce_after_product_thumbnails', array( $this, 'quick_view_view_details_btn'), 60);
			...
			/*Add custom html after product meta*/
			...
			/**Simple Auctions custom mobile btn **/
			add_action( 'woocommerce_single_product_summary', array( $this, 'mobile_auctions_add_btn_after_add_to_cart_form'), 30 ); 

			// Move group button Woo Simple auciton 
			add_action( 'woocommerce_before_bid_form', array( $this, 'woo_simple_move_group_button_auctions'), 30 ); 

			// Move group button yith auciton 
			add_action( 'yith_wcact_before_add_to_cart_form', array( $this, 'yith_move_group_button_auctions'), 30 ); 
		} 
		...
		public function besa_after_meta_single_product() {
			$sidebar_id = besa_tbay_get_config('after_meta_single_products_sidebar');

			if (is_active_sidebar($sidebar_id)) { ?> 
				<aside id="sidebar-after-meta-single-product" class="sidebar after-meta-single-product">
		...
		public function woo_simple_move_group_button_auctions() {
			...
			remove_action('woocommerce_after_add_to_cart_button', array( $this, 'product_group_buttons'), 20);
			add_action('woocommerce_after_bid_form', array( $this, 'product_group_buttons'), 8);
		} 


		public function yith_move_group_button_auctions() {
			if( !class_exists('YITH_Auctions') ) return;
			global $product;
			if ($product->get_type() !== 'auction' ) {
				return;
			}
			...
			remove_action('woocommerce_after_add_to_cart_button', array( $this, 'product_group_buttons'), 20);
			add_action('yith_wcact_before_add_button_bid', array( $this, 'product_group_buttons'), 30);
		} 
		...
		public function buy_now_html() {
			if( besa_catalog_mode_active() ) return;
	        ...
	        if ( $product->get_type() == 'external' || $product->get_type() == 'auction' ) { 
	            return;
	        }
	        ...
	        echo '<button class="'. esc_attr($class) .'">'. esc_html__('Buy Now', 'besa') .'</button>';
		...
		public function the_list_images_review() {
			...
			wp_enqueue_style('photoswipe-ui-default');
	        ...
	        if (is_array($comments) || is_object($comments)) {
				...
				$ptreviews = $this->photo_reviews_thumbnail_image();
				$width_li  =  $ptreviews['width'] + 2; 

				$style = 'style="width: '. esc_attr($width_li) .'px" ';
	                ...
	                if (is_array($image_post_ids) || is_object($image_post_ids)) {
	                    foreach ( $image_post_ids as $image_post_id ) {
	                        if ( ! wc_is_valid_url( $image_post_id ) ) {
	                            ...
	                            $outputs .= '<li '. $style .' class="review-item"><a class="lightbox-gallery" data-caption="'. esc_attr($caption) .'" data-width="'. esc_attr($width) .'" data-height="'. esc_attr($height) .'"  href="'. esc_url($img_src_open) .'"><img class="review-images"
	            ...
	            if ($i > 6) {
	            	if ( ($product_single_layout === 'left-main') || ($product_single_layout === 'main-right') ) {
	            		$i      = $i - 6;
		            	$more   = '<li '. $style .' class="more d-none d-xl-flex">';
		                $more  .= '<span>'. $i .'+</span>';
		                $more  .= '</li>';
		            } elseif ($i > 8) {
		            	$i      = $i - 8;
		...
		public function class_single_product( $styles ) {
			...
			$cart_style = besa_get_mobile_form_cart_style();
		...
		public function the_product_single_time_countdown() {
			...
			$title_time = $skin === 'style1' ? esc_html__('Deal end in: ','besa') : esc_html__('End in','besa');
	        ...
	        <?php if ( $time_sale ): ?>
	            <div class="tbay-time-wrapper">
	              <div class="time tbay-time">
	                  <div class="title"><?php echo trim($title_time); ?></div>
	              ...
	              <?php if($product->get_manage_stock()) {?>
	                <div class="stock">
						...
						<?php if( isset($percentsold) && $skin === 'style1' ) { ?>
	                    <div class="progress">
		...
		public function the_sticky_menu_bar() {
			global $post, $product;
			  ...
			  $sticky_class = ( $product->get_type() === 'auction' ) ? 'auction' : '';

			  

			  if( $product->get_type() === 'auction' ) {
				$sticky_class = apply_filters('besa_sticky_menu_bar_class_auction', 'auction');
			  }
		      ?>
		      <div id="sticky-menu-bar" class="sticky-menu-bar <?php echo esc_attr($sticky_class); ?>">
		        <div class="container">
		          <div class="row">
		...
		public function the_sticky_menu_bar_custom_add_to_cart() {
		    ...
		    } elseif ( $product->get_type() !== 'auction') { 
		      ?> 
		        <a id="sticky-custom-add-to-cart" href="javascript:void(0);"><?php echo esc_html( $product->single_add_to_cart_text() ); ?></a>
		    <?php 
		   } elseif( $product->get_type() === 'auction' && class_exists('YITH_Auctions') && !$product->is_start() ) {
			?>
			<h4 class="yith-not-available-message"><?php echo apply_filters('yith_wcact_auction_not_available_message', esc_html__('This auction has not been started yet', 'besa'),$product) ?></h4>
			<?php
		   }
		}
		...
		public function mobile_add_add_to_cart_button_content() {
			...
			if( besa_get_mobile_form_cart_style() === 'default' ) return;

			global $product;
			?>
			<div id="mobile-close-infor"><i class="<?php echo besa_get_icon('icon_close'); ?>"></i></div>
				...
				<div class="media">
					...
					<div class="media-body">
						<div class="infor-body">
							<?php $this->besa_mobile_infor_body( $product ); ?>
						</div> 
					</div>
				</div>
			</div>
		...
		public function besa_mobile_infor_body( $product ) {
            $this->besa_mobile_infor_body_content( $product );
        }

        public function besa_mobile_infor_body_content( $product ) {
		...
		public function mobile_auctions_add_btn_after_add_to_cart_form() {
			if( !besa_active_woocommerce_auction() ) return;

			global $product;
			$skin = besa_tbay_get_theme(); 

	        if ( $product->get_type() !== 'auction' || $product->is_closed() ) return;
 
			if( class_exists('YITH_Auctions') && !$product->is_start() ) return;

			$class = '';

			$class = $this->mobile_auctions_buy_now_bid_hasclass($product, $class);
			...
			?>
			<div id="mobile-auctions-close-infor"><i class="<?php echo besa_get_icon('icon_close'); ?>"></i></div>
			<div id="mobile-auctions-close-infor-wrapper"></div>
			<div class="mobile-auctions-btn-click <?php echo esc_attr($class); ?>">
				<?php if ($skin === 'style1') {
					?>
						<?php 
							if( ( class_exists('WooCommerce_simple_auction') && $product->is_started() ) || ( class_exists('YITH_Auctions') &&  $product->is_start() && !$product->is_closed() )  ) {
								$this->mobile_auctions_add_to_bid();
							} 
						?>
						
						<?php $this->mobile_auctions_buy_now_bid($product); ?>
					<?php
				} else {
					?>
						<?php $this->mobile_auctions_buy_now_bid($product); ?>
		...
		private function mobile_auctions_add_to_bid() {
			?>
			<button id="tbay-click-addtobid"><?php esc_html_e('Add to bid', 'besa') ?></button>
			<?php
		}

		private function mobile_auctions_buy_now_bid( $product ) {
			if( class_exists('WooCommerce_simple_auction') ) {
				$this->mobile_woo_auctions_buy_now_bid( $product );
			} elseif( class_exists('YITH_Auctions') ) {
				$this->mobile_yith_auctions_buy_now_bid( $product );
			}
		}

		private function mobile_woo_auctions_buy_now_bid( $product ) {
			if ( ! $product->is_purchasable() OR ! $product->is_sold_individually() OR ! $product->is_in_stock() OR $product->is_closed() ) return;
			$this->mobile_auctions_buy_now_bid_html();
		}

		private function mobile_yith_auctions_buy_now_bid( $product ) {
		...
		private function mobile_auctions_buy_now_bid_html() {
			?>
			<button id="tbay-click-buy-now-bid"><?php esc_html_e('Buy Now', 'besa') ?></button>
			<?php
		}

		private function mobile_auctions_buy_now_bid_hasclass( $product, $class ) {
			if ( ! $product->is_purchasable() OR ! $product->is_sold_individually() OR ! $product->is_in_stock() OR $product->is_closed() ) return $class;
			
			$class .= ' has-buy-now-bid';
		...
		public function mobile_add_btn_after_add_to_cart_form() {
			...
			$class = '';
			if( besa_tbay_get_config('enable_buy_now', false) ) {
				$class .= ' has-buy-now';
			}  
			...
			<div class="mobile-btn-cart-click <?php echo esc_attr($class); ?>">
				<?php if ($skin === 'style1') {
					?>
						<div id="tbay-click-addtocart"><?php esc_html_e('Add to cart', 'besa') ?></div>
						<?php if( besa_tbay_get_config('enable_buy_now', false) ) : ?>
							<div id="tbay-click-buy-now"><?php esc_html_e('Buy Now', 'besa') ?></div>
						<?php endif; ?>
					<?php
				} else {
					?>
		...
		public function mobile_add_before_add_to_cart_form( ) {
			...
			foreach ( $attributes as $key => $value ) {
				...
				if( isset($selected_attributes[$key]) && !empty($selected_attributes[$key]) )  {
					...
					$default = esc_html__('Choose an option ', 'besa');
				}
			...
			<div class="mobile-attribute-list">
				...
				<div id="attribute-open"><i class="<?php echo besa_get_icon('icon_attr_open') ?>"></i></div>
			</div>
			<?php
		}
		...
		public function quick_view_view_details_btn() {
			...
			echo '<div class="details-btn-wrapper"><a class="view-details-btn" href="'. esc_url($permalink) .'">'. esc_html__('View details', 'besa') .'</a></div>';
		}
