		...
		$output 	= '<div class="mobile-logo">';
			if( isset($mobilelogo['url']) && !empty($mobilelogo['url']) ) { 
				...
				$output 		.= '<div class="logo-theme">';
					$output 	.= '<a href="'. esc_url( home_url( '/' ) ) .'">';
					$output 	.= '<img class="logo-mobile-img" src="'. esc_url_raw( BESA_IMAGES.'/mobile-logo.png') .'" alt="'. get_bloginfo( 'name' ) .'">';
					$output 	.= '</a>';
				$output 		.= '</div>';
			}
		$output 	.= '</div>';
...
if ( ! function_exists( 'besa_the_icon_mini_cart_mobile' ) ) {
	function besa_the_icon_mini_cart_mobile() {
		...
		if( !defined('BESA_WOOCOMMERCE_ACTIVED') || besa_catalog_mode_active() ) return;
		?>
		<?php 
			if($skin === 'style1') {
				?>
						...
						<div class="tbay-topcart">
							<div id="cart-<?php echo esc_attr($_id); ?>" class="cart-dropdown dropdown">
								<a class="dropdown-toggle mini-cart v2" data-offcanvas="offcanvas-right" data-toggle="dropdown" aria-expanded="true" role="button" aria-haspopup="true" data-delay="0" href="#">
									<i class="<?php echo besa_get_icon('icon_cart_mobile') ?>"></i>
				...
				?>
					<div class="tbay-element-mini-cart">
...
if ( ! function_exists( 'besa_the_icon_mini_cart_header_mobile' ) ) {
	function besa_the_icon_mini_cart_header_mobile() {
		$skin = besa_tbay_get_theme(); 
		if( !besa_catalog_mode_active() && defined('BESA_WOOCOMMERCE_ACTIVED') && (is_product() || is_cart() || is_checkout()) && $skin ==='style1' ) {
			besa_the_icon_mini_cart_mobile();
		} elseif( !besa_catalog_mode_active() && defined('BESA_WOOCOMMERCE_ACTIVED') && is_product()  && $skin ==='style2' ) {
			besa_the_icon_mini_cart_mobile();
		}
	}
}
 ...
 * ------------------------------------------------------------------------------------------------
 */
if ( ! function_exists( 'besa_the_icon_checkout_footer_mobile' ) ) {
	function besa_the_icon_checkout_footer_mobile() {
		if( !defined('BESA_WOOCOMMERCE_ACTIVED') || besa_catalog_mode_active() ) return;
		?>
			<div class="device-checkout">
				<a href="<?php echo esc_url( wc_get_checkout_url() );?>" class="mobile-checkout">
					<i class="tb-icon tb-icon-zt-credit-card"></i>
					<span><?php echo esc_html__('Checkout','besa') ?></span>
				</a>
			</div>
...
/**
 * ------------------------------------------------------------------------------------------------
 * The wishlist header mobile
 * ------------------------------------------------------------------------------------------------
 */
if ( ! function_exists( 'besa_tbay_get_icon_wishlist_header_mobile' ) ) {
	function besa_tbay_get_icon_wishlist_header_mobile() {
		$output = '';
		...
		$output	 .= '<div class="device-wishlist-header">';
		$output  .= '<a class="text-skin wishlist-icon" href="'. esc_url($wishlist_url) .'" >';
		$output  .= apply_filters( 'besa_get_mobile_wishlist_icon', '<i class="'.besa_get_icon('icon_wishlist_mb').'"></i>', 2 );
		$output  .= '<span class="count count-wishlist">'. esc_html($wishlist_count) .'</span>';
		$output  .='</a>';
		$output  .='</div>';
		
		return apply_filters( 'besa_tbay_get_icon_wishlist_header_mobile', $output , 10 );
	}
}

if ( ! function_exists( 'besa_the_icon_wishlist_header_mobile' ) ) {
	function besa_the_icon_wishlist_header_mobile() {
		$ouput = besa_tbay_get_icon_wishlist_header_mobile();
	...
	function besa_top_header_mobile() { ?>
		<div class="top-right-mobile">
			<?php 
				...
				*/
				if ($skin === 'style2') {
					add_action('besa_top_header_mobile', 'besa_the_icon_wishlist_header_mobile', 10);
				}
...
if ( ! function_exists( 'besa_the_icon_home_page_mobile' ) ) {
	function besa_the_icon_home_page_mobile() {
		...
		if( !besa_catalog_mode_active() && defined('BESA_WOOCOMMERCE_ACTIVED') && (is_product() || is_cart() || is_checkout()) ) {
...
if ( ! function_exists( 'besa_the_hook_header_mobile_menu_all_page_2' ) ) {
	function besa_the_hook_header_mobile_menu_all_page_2() {
		$menu_mobile_all_page_2 	= besa_tbay_get_config('menu_mobile_all_page_2', false);
		remove_action( 'besa_header_mobile_content', 'besa_the_back_mobile', 5 );	

		if ($menu_mobile_all_page_2) {
...
if ( ! function_exists( 'besa_tbay_get_icon_wishlist_footer_mobile' ) ) {
	function besa_tbay_get_icon_wishlist_footer_mobile() {
		...
		$output  .= '<a class="text-skin wishlist-icon" href="'. esc_url($wishlist_url) .'" >';
		$output  .= apply_filters( 'besa_get_mobile_wishlist_icon', '<i class="'.besa_get_icon('icon_wishlist').'"></i>', 2 );
...
if ( ! function_exists( 'besa_tbay_get_icon_account_footer_mobile' ) ) {
	function besa_tbay_get_icon_account_footer_mobile() {
		...
		if ( besa_catalog_mode_active() || !defined('BESA_WOOCOMMERCE_ACTIVED') ) return $output;
