...
if ( ! function_exists( 'besa_tbay_post_info_author' ) ) {
	function besa_tbay_post_info_author() { 
        global $authordata;
        if ( ! is_object( $authordata ) || get_post_type( get_the_ID() ) == 'job_listing' ) {
            return '';
        }
		...
		<div class="author-info">
			...
			<div class="content">
				...
				<a href="<?php echo esc_url( get_author_posts_url( $authordata->ID, $authordata->user_nicename ) ); ?>" class="all-post"><?php esc_html_e('See all author post ', 'besa'); ?><?php echo  '<i class="tb-icon tb-icon-chevron-right"></i>' ?></a>
			</div>
		<?php } 
	}
...
if ( ! function_exists( 'besa_tbay_paging_nav' ) ) {
	function besa_tbay_paging_nav() {
		...
		<nav class="navigation paging-navigation">
			<h5 class="screen-reader-text hidden"><?php esc_html_e( 'Posts navigation', 'besa' ); ?></h5>
...
if (!function_exists('besa_get_elementor_css_print_method') ) {
	function besa_get_elementor_css_print_method() {
...
if (!function_exists('besa_get_header_id') ) {
	function besa_get_header_id() {
...
if (!function_exists('besa_get_display_header_builder') ) {
	function besa_get_display_header_builder() {
		$id = besa_get_header_id();
		
		return  besa_get_html_custom_post($id);
	}
}
...
if (!function_exists('besa_get_display_footer_builder') ) {
	function besa_get_display_footer_builder($footer) {
		...
		return  ( !empty($posts[0]->ID) ) ? besa_get_html_custom_post($posts[0]->ID) : '';
	}
}
...
if( ! function_exists( 'besa_get_html_custom_post' ) ) {
	function besa_get_html_custom_post($id) {
        if( is_null($id) || empty($id) ) return;
        
        $post = get_post( $id );

        if ( besa_elementor_is_activated() && Elementor\Plugin::instance()->documents->get( $id )->is_built_with_elementor() ) {
            return Elementor\Plugin::instance()->frontend->get_builder_content_for_display($id, besa_get_elementor_css_print_method());
        } else {
...
if ( ! function_exists( 'besa_tbay_woocs_redraw_cart' ) ) {
    function besa_tbay_woocs_redraw_cart() {
        return 0;
    }
    add_filter( 'woocs_redraw_cart', 'besa_tbay_woocs_redraw_cart', 10 ,1 );
}

if( ! function_exists('besa_load_html_dropdowns_action') ) {
	function besa_load_html_dropdowns_action() {
		$response = array(
			'status' => 'error',
		...
		if( isset( $_POST['ids'] ) && is_array( $_POST['ids'] ) ) {
			$ids = besa_clean( $_POST['ids'] );
			foreach ($ids as $id) {
				$id = (int) $id;
				$content = besa_get_html_custom_post($id);
				if( ! $content ) continue;
	...
	add_action( 'wp_ajax_besa_load_html_dropdowns', 'besa_load_html_dropdowns_action' );
	add_action( 'wp_ajax_nopriv_besa_load_html_dropdowns', 'besa_load_html_dropdowns_action' );
}

if( ! function_exists('besa_load_html_click_action') ) {
	function besa_load_html_click_action() {
		$response = array(
			'status' => 'error',
			'message' => 'Can\'t load HTML blocks with AJAX',
			'data' => array(),
		...
		if( ! empty( $_POST['slug'] ) ) {
			$slug 			= besa_clean( $_POST['slug'] );
			$layout 		= besa_clean( $_POST['layout'] );
			...
			$args['menu_class'] = besa_nav_menu_get_menu_class($layout);
	...
	add_action( 'wp_ajax_besa_load_html_click', 'besa_load_html_click_action' );
	add_action( 'wp_ajax_nopriv_besa_load_html_click', 'besa_load_html_click_action' );
}

if ( ! function_exists( 'besa_get_elementor_post_scripts' ) ) {
	function besa_get_elementor_post_scripts() {
		if( !besa_elementor_is_activated() ) return;
		...
		if ( class_exists( '\Elementor\Core\Files\CSS\Post' ) ) {
			$css_file = new \Elementor\Core\Files\CSS\Post( besa_get_header_id() );
		} elseif ( class_exists( '\Elementor\Post_CSS_File' ) ) {
			$css_file = new \Elementor\Post_CSS_File( besa_get_header_id() );
		}
...
if ( !function_exists('besa_tbay_back_to_top') ) {
    function besa_tbay_back_to_top(){
		$skin = besa_tbay_get_theme(); 
		if ($skin === 'style1' && besa_tbay_get_config('back_to_top') ) {
			?>
			...
			<?php
		} elseif( $skin === 'style2' && besa_tbay_get_config('back_to_top') ) {
			?>
			<div class="tbay-to-top">
				<a href="javascript:void(0);" id="back-to-top">
					<i class="tb-icon tb-icon-zt-angle-up"></i>
					<?php echo esc_html__('Back to top','besa') ?>
				</a>
			</div>
			<?php
		}
			...
			<div class="tbay-to-top-mobile tbay-to-top">
				...
				<div class="more-to-top">
					...
					<a href="javascript:void(0);" id="back-to-top-mobile">
						<i class="<?php echo besa_get_icon('icon_back_to_top') ?>"></i>
					</a>
    ...
    add_action('elementor/theme/after_do_footer', 'besa_tbay_back_to_top', 10);
    add_action('besa_after_do_footer', 'besa_tbay_back_to_top', 10);
}