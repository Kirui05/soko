...
require get_theme_file_path('inc/merlin-data/data-version-v1.php');
require get_theme_file_path('inc/merlin-data/data-version-v2.php');
require get_theme_file_path('inc/merlin-data/data-version-auctions.php');
...
- - - - - - - - - - - - - - - - - - - - */";
    ...
    public function after_import_setup($selected_import)
    {
        ...
        if (!isset($check_oneclick[$selected_import['home']]) || apply_filters('besa_reset_import_rev_sliders', false)) {
    ...
    private function import_revslider($revsliders)
    {
        if (class_exists('RevSliderAdmin')) {
            ...
            $my_filesystem = new WP_Filesystem_Direct([]);
    ...
    public function setup_options_after_import()
    {
        ...
        /**Auctions**/
        $this->update_option_yith_auctions();
        $this->update_option_woo_simple_auctions();
    }

    private function update_option_yith_auctions()
    {
        if (!class_exists('YITH_Auctions') || !class_exists('YITH_Auctions_Premium')) {
            return;
        }

        update_option('yith_wcact_show_countdown_in_loop', 'yes');
        update_option('yith_wcact_settings_show_bid_increment_in_the_page', 'yes');
        update_option('yith_wcact_settings_enable_watchlist', 'yes');
    }

    private function update_option_woo_simple_auctions()
    {
        ...
        update_option('simple_auctions_compact_countdown', 'yes');
    ...
    private function update_option_woof() {
        ...
        $settings['custom_tax_label']['product_cat'] = esc_html__('Categories', 'besa');
        ...
        if (class_exists('YITH_WCBR')) {
            ...
            $settings['tax_type']['yith_product_brand'] = 'checkbox';
    ...
    private function update_option_mvx()
    {
        ...
        $settings_name = get_option('mvx_general_settings_name', []);
        ...
        $vendor_name = get_option('mvx_vendor_general_settings_name', []);
        ...
        $capabilities = get_option('mvx_capabilities_product_settings_name', []);
    ...
    public function import_files_v1()
    {
        $data = new Besa_Merlin_Data_Version_V1();

        $prefix = $this->import_files_type_demo();

        if (is_rtl()) {
            return $data->import_files_rtl();
        }
    ...
    public function import_files_v2()
    {
        $data = new Besa_Merlin_Data_Version_V2();
        ...
        $dark_mode = $data->import_files_dark_mode();

        switch ($prefix) {
            case 'dokan':
                return array_merge($data->import_files_dokan(), $dark_mode);
                break;

            case 'wcfm':
                return array_merge($data->import_files_wcfm(), $dark_mode);
                break;

            case 'mvx':
                return array_merge($data->import_files_mvx(), $dark_mode);
                break;

            case 'wcvendors':
                return array_merge($data->import_files_wcvendors(), $dark_mode);
                break;
    ...
    public function import_files_auctions()
    {
        $data = new Besa_Merlin_Data_Version_Auctions();

        if (class_exists('WooCommerce_simple_auction')) {
            return $data->import_files_woo_simple_auction();
        } elseif (class_exists('YITH_Auctions')) {
            return $data->import_files_yith_auction();
        } else {
            return '';
        }
    }
    ...
    {
        $data_v1 = $this->import_files_v1();
        $data_v2 = $this->import_files_v2();
        $data_auctions = $this->import_files_auctions();

        if (!empty($data_auctions)) {
            return array_merge($data_auctions, $data_v2, $data_v1);
        } else {
            return array_merge($data_v2, $data_v1);
        }
    }
}
