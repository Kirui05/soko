...
<table class="shop_table cart wishlist_table wishlist_view traditional responsive" data-pagination="<?php echo esc_attr( $pagination )?>" data-per-page="<?php echo esc_attr( $per_page )?>" data-page="<?php echo esc_attr( $current_page )?>" data-id="<?php echo esc_attr($wishlist_id); ?>" data-token="<?php echo esc_attr($wishlist_token); ?>">
    ...
    if( $wishlist && $wishlist->has_items() ) :
                ...
                <tr id="yith-wcwl-row-<?php echo esc_attr($item->get_product_id()); ?>" data-row-id="<?php echo esc_attr($item->get_product_id()); ?>">
                    ...
                    <?php if( $repeat_remove_button ): ?>
                        <td class="product-remove">
                            <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item->get_product_id() ) ) ?>" class="remove_from_wishlist" title="<?php echo apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', esc_html__( 'Remove this product', 'besa' ) ); ?>"><i class="<?php echo besa_get_icon('icon_remove') ?>"></i><?php esc_html_e( 'Remove', 'besa' ) ?></a>
                        </td>
                    <?php endif; ?>
        ...
        <?php 
            ...
            $icon = $skin === 'style2' ? '<i class="tb-icon tb-icon-zt-box"></i>' : '';
        ?>
        <tr class="no-products">
            <td colspan="<?php echo esc_attr( $column_count ) ?>" class="wishlist-empty"> <?php echo trim($icon); ?> <?php echo apply_filters( 'yith_wcwl_no_product_to_remove_message', esc_html__( 'No products added to the wishlist', 'besa' ) ) ?></td>
        </tr>
    <?php
    endif;
